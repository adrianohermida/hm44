...
export function useFiltrosTicket() {
  ...
  const { data: filtros = [], isLoading } = useQuery({
    queryKey: ['filtros-salvos-ticket', user?.email],
  ...
  const favoritarMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries(['filtros-salvos-ticket'])
  });

  const deletarMutation = useMutation({
    mutationFn: (id) => base44.entities.FiltroSalvo.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries(['filtros-salvos-ticket']);
